...
func doUpdateSpotOceanCluster(cmd *cmdutils.Cmd) error {
	...
	if err := ctl.LoadClusterIntoSpecFromStack(ctx, cfg, stackManager); err != nil {
	...
	if err := nodeGroupService.Normalize(ctx, []api.NodePool{ng}, meta); err != nil {
		return err
	}
	...
	if !cmd.Plan {
		if err := stackManager.UpdateNodeGroupStack(ng.Name, string(existingTemplateJSON), cmd.Wait); err != nil {
