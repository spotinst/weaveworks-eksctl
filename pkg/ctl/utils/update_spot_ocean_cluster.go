...
func doUpdateSpotOceanCluster(cmd *cmdutils.Cmd) error {
	...
	ng := spot.NewOceanClusterNodeGroup(meta)
	ng.SpotOcean = cfg.SpotOcean
	...
	if err := nodeGroupService.Normalize([]api.NodePool{ng}); err != nil {
		return err
	}
	...
	newStack := builder.NewNodeGroupResourceSet(ctl.Provider.EC2(), ctl.Provider.IAM(),
		cfg, ng, stack.Tags, false, false, vpcImporter)
	...
	existingNodeGroup.Properties[api.SpotOceanNodeGroupName] =
		newNodeGroupCFN.Properties[api.SpotOceanNodeGroupName]
