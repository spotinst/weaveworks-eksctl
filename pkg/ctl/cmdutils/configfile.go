...
func NewCreateNodeGroupLoader(cmd *Cmd, ng *api.NodeGroup, ngFilter *filter.NodeGroupFilter, managedNodeGroup, spotOcean bool, spotProfile string) ClusterConfigLoader {
	l := newCommonClusterConfigLoader(cmd)
	...
	l.validateWithConfigFile = func() error {
		...
		// Spot Ocean.
		{
			if ng.SpotOcean != nil && spotProfile != "" {
				if ng.SpotOcean.Metadata == nil {
					ng.SpotOcean.Metadata = new(api.NodeGroupSpotOceanMetadata)
				}
				ng.SpotOcean.Metadata.Profile = &spotProfile
			}
		}
