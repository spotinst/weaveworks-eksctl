...
func doDeleteNodeGroup(cmd *cmdutils.Cmd, ng *api.NodeGroup, params *cmdutils.DeleteNodeGroupCmdParams) error {
	...
	// Spot Ocean.
	{
		...
		if err := spot.RunPreDelete(ctl.Provider, cfg, nodeGroups, stacks,
	...
	if params.Drain && !params.SpotRoll {
		...
		err := nodeGroupManager.Drain(allNodeGroups, cmd.Plan, params.MaxGracePeriod, 0, false, params.DisableEviction)
		if err != nil {
