...
func doDeleteNodeGroup(cmd *cmdutils.Cmd, ng *api.NodeGroup, params *cmdutils.DeleteNodeGroupCmdParams) error {
	...
	// Spot Ocean.
	{
		...
		nodeGroupsIncludedFilter := spot.NewDeleteIncludedFilter(nodeGroups)
	...
	if params.Drain && !params.SpotRoll {
		err := nodeGroupManager.Drain(allNodeGroups, cmd.Plan, params.MaxGracePeriod, params.DisableEviction)
		if err != nil {
			return err
		}
	}
