...
func deleteNodeGroupWithRunFunc(cmd *cmdutils.Cmd, runFunc func(cmd *cmdutils.Cmd, ng *api.NodeGroup, options deleteNodeGroupOptions) error) {
	...
	cmd.FlagSetGroup.InFlagSet("Spot Ocean", func(fs *pflag.FlagSet) {
		cmdutils.AddSpotOceanDeleteNodeGroupFlags(fs, &options.spotRoll, &options.spotRollBatchSize)
	})
}
...
func doDeleteNodeGroup(cmd *cmdutils.Cmd, ng *api.NodeGroup, options deleteNodeGroupOptions) error {
	...
	// Spot Ocean.
	{
		...
		if err := spot.RunPreDelete(ctx, ctl.AWSProvider, cfg, nodeGroups, stacks,
			nodeGroupsIncludedFilter, options.spotRoll, options.spotRollBatchSize, cmd.Plan); err != nil {
			return err
		}
	...
	if options.deleteNodeGroupDrain && !options.spotRoll {
