...
func RunPreDelete(provider api.ClusterProvider, config *api.ClusterConfig,
	nodeGroups []*api.NodeGroup, stacks []*cloudformation.Stack,
	...
	if !plan {
		...
		if AllowCredentialsChanges.Enabled() {
			...
			for _, ng := range oceanNodeGroups {
				if err := UpdateCredentials(provider, ng, stacks); err != nil {
					return err
				}
			}
		}
	...
	if stack != nil {
		if !plan && AllowCredentialsChanges.Enabled() {
			ng := NewOceanClusterNodeGroup(config.Metadata)
			if err = UpdateCredentials(provider, ng, stacks); err != nil {
				return err
			}
		}
		...
		config.NodeGroups = append(config.NodeGroups,
			NewOceanClusterNodeGroup(config.Metadata))
	}
...
func ShouldCreateOceanCluster(config *api.ClusterConfig,
	...
	if config.SpotOcean != nil { // explicit
		ng = NewOceanClusterNodeGroup(config.Metadata)
	} else { // implicit
...
func ShouldDeleteOceanCluster(stacks []*cloudformation.Stack,
	shouldDelete FilterFunc) (*cloudformation.Stack, error) {
...
// stack parameters.
func UpdateCredentials(provider api.ClusterProvider,
	...
	if err := updateStackCredentials(provider, stack, token, account); err != nil {
		return err
	}
...
func updateStackCredentials(provider api.ClusterProvider,
	...
	template, err := updateTemplateCredentials(provider, stack, token, account)
	if err != nil {
		return err
	}
...
func updateTemplateCredentials(provider api.ClusterProvider,
	...
	output, err := provider.CloudFormation().GetTemplate(input)
	if err != nil {
