...
func RunPreDelete(ctx context.Context, provider api.ClusterProvider,
	clusterSpec *api.ClusterConfig, nodeGroups []*api.NodeGroup, stacks []*cloudformation.Stack,
...
func ShouldCreateOceanCluster(clusterSpec *api.ClusterConfig, stacks []*cloudformation.Stack) *api.NodeGroup {
...
func updateTemplateCredentials(provider api.ClusterProvider,
	...
	if _, ok := template.Parameters[CredentialsTokenParameterKey]; ok {
		template.Parameters[CredentialsTokenParameterKey] = token
	}

	if _, ok := template.Parameters[CredentialsAccountParameterKey]; ok {
		template.Parameters[CredentialsAccountParameterKey] = account
	}

	if _, ok := template.Parameters[FeatureFlagsParameterKey]; ok {
		template.Parameters[FeatureFlagsParameterKey] = LoadFeatureFlags()
	}
