...
func NewOceanVirtualNodeGroup() *api.NodeGroup {
	ng := api.NewNodeGroup()
	ng.SpotOcean = new(api.SpotOcean)
	return ng
}

// NewOceanClusterNodeGroup creates a new Ocean Cluster nodegroup, and returns
// a pointer to it.
func NewOceanClusterNodeGroup(meta *api.ClusterMeta) *api.NodeGroup {
...
func RunPreDelete(clusterProvider api.ClusterProvider,
	clusterConfig *api.ClusterConfig, nodeGroups []*api.NodeGroup,
	stacks []*cloudformation.Stack, shouldDelete FilterFunc,
	...
	if !plan {
		...
		if AllowCredentialsChanges.Enabled() {
			...
			for _, ng := range oceanNodeGroups {
				if err := UpdateCredentials(clusterProvider, ng, stacks); err != nil {
					return err
				}
			}
		}
	...
	if stack != nil {
		if !plan && AllowCredentialsChanges.Enabled() {
			ng := NewOceanClusterNodeGroup(clusterConfig.Metadata)
			if err = UpdateCredentials(clusterProvider, ng, stacks); err != nil {
				return err
			}
		}
		...
		clusterConfig.NodeGroups = append(clusterConfig.NodeGroups,
			NewOceanClusterNodeGroup(clusterConfig.Metadata))
	}
...
// ShouldCreateOceanNodeGroup checks whether the Ocean Cluster's nodegroup
// should be created and returns its configuration.
func ShouldCreateOceanNodeGroup(clusterConfig *api.ClusterConfig,
	nodeGroups []*api.NodeGroup, stacks []*cloudformation.Stack) *api.NodeGroup {
	...
	if clusterConfig.SpotOcean != nil { // explicit
		ng = NewOceanClusterNodeGroup(clusterConfig.Metadata)
		ng.SpotOcean = clusterConfig.SpotOcean
	} else { // implicit
...
// ShouldDeleteOceanNodeGroup checks whether the Ocean Cluster's nodegroup
// should be deleted and returns its CloudFormation stack.
...
func getOceanClusterIDFromStacks(stacks []*cloudformation.Stack) (clusterID string) {
...
func getOceanVirtualNodeGroupIDFromStacks(
	stacks []*cloudformation.Stack, ngName string) (vngID string) {
	collectors := map[string]outputs.Collector{
...
// stack parameters.
func UpdateCredentials(clusterProvider api.ClusterProvider,
	ng *api.NodeGroup, stacks []*cloudformation.Stack) error {
	...
	if err := updateStackCredentials(clusterProvider, stack, token, account); err != nil {
		return err
	}
...
func updateStackCredentials(clusterProvider api.ClusterProvider,
	...
	template, err := updateTemplateCredentials(clusterProvider, stack, token, account)
	if err != nil {
		return err
	}
...
func updateTemplateCredentials(clusterProvider api.ClusterProvider,
