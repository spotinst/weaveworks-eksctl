...
func RunPreDeletion(clusterProvider api.ClusterProvider,
	...
	// Filter nodegroups that are marked for deletion.
	...
	if stack != nil {
		...
		// Allow post-deletion actions on Ocean cluster's nodegroup.
...
func rollingUpdate(nodeGroups []*api.NodeGroup,
	...
	logger.Debug("ocean: rolling launch specs: %s", strings.Join(specIDs, "; "))
